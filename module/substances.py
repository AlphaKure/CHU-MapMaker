...
def SubstanceTagMaker(FileType:str,NowPart:str,Type:int)->BeautifulSoup:
    ...
    if FileType=='Reward':
        ...
        if NowPart=='gamePoint':
            if Type==0:
                _sAmount=input(Fore.GREEN+Content['Cross']['Input']['Input_GamePoint']+Fore.RESET)
            else:
                _sAmount='0'
        ...
        #skillSeed-reward
            ...
            if Type==4:
                _sSkillseedId=input(Fore.GREEN+Content['Cross']['Input']['Input_SkillSeedID']+Fore.RESET)
                _sSkillseedcount=input(Fore.GREEN+Content['Cross']['Input']['Input_SkillSeedCount']+Fore.RESET)
                if checkconfig('AutoStr','Skill'):
                    ...
                    _sSkillseedStr=input(Fore.GREEN+Content['Cross']['Input']['Input_SkillSeedStr']+Fore.RESET)
            else:
                _sSkillseedId='-1'
        ...
        #Other-reward 公版    
            ...
            if NowPart==TagNameList[Type]:
                Id=input(Fore.GREEN+Content['Cross']['Input']['Input_ID'].replace('%TagName%',TagNameList[Type])+Fore.RESET)
                if checkconfig('AutoStr',TagNameList[Type]):
                    _fFInd,Str=getstr('name','AutoStr',TagNameList[Type],Id)
                    if not _fFInd:
                        return SubstanceTagMaker(FileType,NowPart,Type)
                else:
                    Str=input(Fore.GREEN+Content['Cross']['Input']['Input_Str'].replace('%TagName%',TagNameList[Type])+Fore.RESET)
            else:
                Id='-1'
                Str='Invalid'
        ...
        if NowPart=='musicDif':
            if Type==7:
                Point=input(Fore.GREEN+Content['Cross']['Input']['Input_MapBonusPoint']+Fore.RESET)
                Id=input(Fore.GREEN+Content['Cross']['Input']['Input_MapBonusMusicDif']+Fore.RESET)
                Str=musicDif(int(Id))
            else:
                Point='1'
                Id='-1'
            ...
            if Type==8:
                Point=input(Fore.GREEN+Content['Cross']['Input']['Input_MapBonusPoint']+Fore.RESET)
                Lv=input(Fore.GREEN+Content['Cross']['Input']['Input_MapBonusMusicLv']+Fore.RESET)
                if Lv.endswith('+'):
                    Id=str(int(Lv[:-1])*2)
                    Str='ID_'+Id
                    Data='Lv'+Lv
            ...
            if Type==3:
                Point=input(Fore.GREEN+Content['Cross']['Input']['Input_MapBonusPoint']+Fore.RESET)
                Id=input(Fore.GREEN+Content['Cross']['Input']['Input_MapBonusTriggerID'].replace('%TagName%',TagNameList[Type])+Fore.RESET)
                if checkconfig('AutoStr',FileNameList[Type]):
                    _fFind,Str=getstr(FindKeyList[Type],'AutoStr',FileNameList[Type],Id)
                    if not _fFind:
                        return SubstanceTagMaker(FileType,NowPart,Type)
                else:
                    Str=input(Fore.GREEN+Content['Cross']['Input']['Input_MapBonusTriggerStr'].replace('%TagName%',TagNameList[Type])+Fore.RESET)
            else:
                Point='1'
                Id='-1'
                Str='Invalid'
        ...
        if NowPart=='music':
            _sCourseMusic=input(Fore.GREEN+Content['Cross']['Input']['Input_MapHaveTaskTrack']+Fore.RESET).lower()
            if _sCourseMusic=='n':
                Id='-1'
                Str='Invalid'
                isHard='false'
            else:
                Id=input(Fore.GREEN+Content['Cross']['Input']['Input_TaskTrackID']+Fore.RESET)
                if checkconfig('AutoStr','music'):
                    _fFInd,Str=getstr('name','AutoStr','music',Id)
                    if not _fFInd:
                        return SubstanceTagMaker(FileType,NowPart,Type)
                else:
                    Str=input(Fore.GREEN+Content['Cross']['Input']['Input_TaskTrackStr']+Fore.RESET)
                    isHard=input(Fore.GREEN+Content['Cross']['Input']['Input_TaskTrackDifLock']+Fore.RESET).lower()
                    if isHard=='y':
                        isHard='true'
                    else:
                        isHard='false'
