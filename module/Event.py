...
def EventM(Content):
    ...
    if not checkconfig('SavePath', 'EventPath'):
        ...
        _tName = tagM('<name><id>'+_sEventNum+'</id><str>' +
                      _sEventStr+'</str><data/></name>')
        if len(_sEventNum) > 8:
            print(Fore.RED+Content['Cross']['Error_Msg']
        ...
        if input(Fore.GREEN+Content['Event']['Input']['AddDdsBanner']+Fore.RESET).lower() == 'y':
            ...
            if checkconfig('AutoStr', 'ddsBanner'):
                _fFInd, _sDdsBannerStr = getstr(
                    'name', 'AutoStr', 'ddsBanner', _sDdsBannerId)
                if not _fFInd:
                    return
            else:
                _sDdsBannerStr = input(
                    Fore.GREEN+Content['Event']['Input']['DdsBannerStr']+Fore.RESET)
        else:
            _sDdsBannerId = '-1'
            _sDdsBannerStr = 'Invalid'
        _dXml.EventData.append(tagM('<ddsBannerName><id>'+_sDdsBannerId +
                               '</id><str>'+_sDdsBannerStr+'</str><data /></ddsBannerName>'))
