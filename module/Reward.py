...
def RewardM(Content):
    if not checkconfig('SavePath', 'RewardPath'):
        ...
        _tName = tagM('<name><id>'+_sRewardNum+'</id><str>' +
                      _sRewardStr+'</str><data/></name>')
        ...
        if _iType > 10:
            ...
            _tSubstances = tagM('<substances><list><RewardSubstanceData><type>' +
                                str(_iType)+'</type></RewardSubstanceData></list></substances>')
            _dXml.RewardData.append(_tSubstances)

            # gamepoint reward
            _tGamepoint = SubstanceTagMaker('Reward', 'gamePoint', _iType)
            _dXml.RewardData.substances.list.RewardSubstanceData.append(
                _tGamepoint)
            ...
            # Skillseed
            _tSkillseed, _tSkillseedcount = SubstanceTagMaker(
                'Reward', 'skillSeed', _iType)
            _dXml.RewardData.substances.list.RewardSubstanceData.append(
                _tSkillseed)
            _dXml.RewardData.substances.list.RewardSubstanceData.skillSeed.append(
                _tSkillseedcount)
            ...
            # AvatarAccessory
            _tAvaterAccessory = SubstanceTagMaker(
                'Reward', 'avatarAccessory', _iType)
            _dXml.RewardData.substances.list.RewardSubstanceData.append(
                _tAvaterAccessory)
